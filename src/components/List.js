...
class List extends Component{
    ...
    getFollowing = ()=>{
        this.props.list.selected = 'following'
        this.props.getListFollowingAsync(this.props.login.name || window.localStorage.getItem('gitHubName'))
    }
    renderUser =(param,index)=>{
        return(
            <div className="d-table table-fixed col-12 width-full py-4 border-bottom border-gray-light" key={index}>
                <div className="d-table-cell col-1 v-align-top">
                    <a className="d-inline-block" href="/fengjianjunstudy">
                        <img className="avatar" src={param.avatar_url} width="50" height="50" alt="@fengjianjunstudy" />
                    </a>
                </div>
                <div className="d-table-cell col-9 v-align-top pr-3">
                    <a className="d-inline-block no-underline mb-1"  href={param.html_url} aria-describedby="hovercard-aria-description">
                        <span className="link-gray pl-1">{param.login}</span>
                    </a>
                </div>
            </div>
        )
    ...
    render(){
        ...
        let repoDom = [],followerDom = [],followingDom = []
        ...
        this.props.list.followers && this.props.list.followers.forEach((follower,index)=>{
            followerDom.push(
                this.renderUser(follower,index)
            )
        })
        this.props.list.following && this.props.list.following.forEach((follow,index)=>{
            followingDom.push(
                this.renderUser(follow,index)
            )
        })

        return(
            <div className="container-lg clearfix px-3 mt-4">
                <div className="float-left col-3 pr-3 h-card">
                ...
                <div className="col-9 float-left pl-2">
                    ...
                    <div className="UnderlineNav user-profile-nav js-sticky top-0" style={{position: 'static', top: '0px', left: '403.5px', width: '727px'}}>
                        <nav className="UnderlineNav-body" data-pjax="" role="navigation">
                            ...
                            <a onClick={this.getFollowing} href="#/list?tab=following" className={`UnderlineNav-item ${this.props.list.selected == 'following'?'selected':''}`} aria-selected="false" role="tab" title="Following">Following<span className="Counter">{info.following}</span>
                            </a>
                        </nav>
                    </div>
                    <div className="position-relative">
                        ...
                        <div id="user-repositories-list" style={{'display':this.props.list.selected == 'following'?'block':'none'}}>
                            {followingDom}
                        </div>
                    </div>
                </div>
